...
def pretrain_sac(delay_type, timesteps, learning_rate, qf_learning_rate, progress_bar, verbose, env_id, gamma, buffer_size):
    # print(f"Delay type: {delay_type} timesteps: {timesteps}")
    assert delay_type in DELAY_CLASSES.keys(), "Invalid delay type"

    policy_kwargs = dict(net_arch=[128, 64]) # Increase architecture a bit..
    ...
    for wrapper in DELAY_CLASSES[delay_type]:
        ...
        model.save(f"pretrained_models/{model_name}")

        del model # To ensure loading works
        # print(f"pretrained_models/{model_name}")
        model = TQC.load(f"pretrained_models/{model_name}")
